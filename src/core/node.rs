...
use tarpc::{
    context, serde::Deserialize, serde::Serialize, server::Channel, tokio_serde::formats::Bincode,
};
...
impl NodeServer {
    pub fn new(node: Node, config: Config) -> Self {
        assert!(
            ...
            "replication_factor equal to 0"
        );
        assert!(
            config.replication_factor <= config.fault_tolerance + 1,
            "replication_factor greater than fault_tolerance + 1"
        );
    ...
    pub async fn start(&mut self, join_node: Option<Node>) -> DhtResult<ServerManager> {
        ...
        let stabilize_handle = tokio::spawn(async move {
            if stabilize_interval > 0 {
                let mut interval =
                    tokio::time::interval(tokio::time::Duration::from_millis(stabilize_interval));
        ...
        let fix_finger_handle = tokio::spawn(async move {
            if fix_finger_interval > 0 {
                let mut interval =
                    tokio::time::interval(tokio::time::Duration::from_millis(fix_finger_interval));
        ...
        let joined_handle =
            future::join_all(vec![listener_handle, stabilize_handle, fix_finger_handle]);
    ...
    pub async fn fix_finger(&mut self, index: usize) {
        match self
            .find_successor_list(self.finger_table_start(index))
            .await
        {
            Ok(succ) => {
    ...
    async fn find_predecessor(&mut self, id: Digest) -> DhtResult<Node> {
        ...
        while !(in_range(id, n.id, succ.id) || id == succ.id) {
            debug!(
                "{}: find_predecessor range ({}, {}]",
                self.node, n.id, succ.id
            );
        ...
        loop {
            for i in 0..(self.config.retry_limit + 1) {
                match self.find_successor_list(id).await {
                    ...
                    Err(e) => {
                        warn!(
                            "{}: find_successor_list_rpc failed (retry {}): {}",
                            self.node, i, e
                        );
        ...
        loop {
            for i in 0..(self.config.retry_limit + 1) {
                match self.find_predecessor(id).await {
                    ...
                    Err(e) => {
                        warn!(
                            "{}: find_predecessor_rpc failed (retry {}): {}",
                            self.node, i, e
                        );
                        tokio::time::sleep(tokio::time::Duration::from_millis(
                            self.config.retry_interval,
